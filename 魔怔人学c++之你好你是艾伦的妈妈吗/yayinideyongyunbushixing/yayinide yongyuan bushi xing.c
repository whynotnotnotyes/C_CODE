...
//	//也就是00000001(数据)
//	if (*pa == 1)  //如果解引用第一个字节是1,那内存中存储的就是10000000(地址),也就是说编译器把低位数据放在了低位的地址
//	{              //如果解引用第一个字节是0,那内存中存储的就是00000001,也就是说编译器把低位数据放在了高位的地址
...
//为什么10000000是-128？10000000-1成反码，但是根本减不了，因为是有符号的char，前面全是0
//但110000000补码转成反码是101111111,再转成原码就是110000000
//原码跟补码相同,补码第一个8比特位和原码第一个8比特一样,所以当有符号遇到10000000,就是转换成-128
//
//10000001 ->-127 
//...
//11111101 ->-3
...
//可以想象成一个圆圈,圆圈的开头是0,在127上加1,就会变成-128,再往上加1就会变成-127,一直往后加又会回到0
...
//但是我们知道，科学计数法中的E是可以出现负数的，所以在IEEE754规定，存入内存时E的真实值必须再加上一个中间数
...
//为什么p1和p2是相同的?因为两个"abcdef"都是常量表达式,不能更改，而且两个字符串一模一样
//内存为了节省空间,只存储了一份
...
//	int y = 0;
//
//	int (*parr[])(int, int) = { 0,add,sub,mul,chu }; //为什么第一个元素要是0？因为不是0的话，选择1其实是数组的第二个元素，也就是sub
...
//#include <stdio.h>
//
//void print(char* str)   //为什么传过来的是"bit",接收却是指针？因为"bit"是字符串常量，传过去的第一个元素的地址，既然是地址，就应该用指针接收
...
int cmo_stu_by_name(const void* e1, const void* e2)
{
	//比较名字就是比较字符串
